...
/**
 * Semver patch segmentini bir artırır. Önek (v...) ve sonek (-beta, +meta)
 * korunur; numerik bir patch bulunamazsa version aynen döner.
 * Örn: "1.0.1" → "1.0.2", "v1.0.1-rc.1" → "v1.0.2-rc.1".
 */
export function bumpPatch(version) {
  const m = /^(\D*)(\d+)\.(\d+)\.(\d+)(.*)$/.exec(version ?? "");
  if (!m) return version;
  const [, prefix, major, minor, patch, suffix] = m;
  return `${prefix}${major}.${minor}.${Number(patch) + 1}${suffix}`;
}

/**
 * Dist branch'inde son commit'lenmiş package.json version'unu döndürür.
 * Worktree'nin çalışma kopyası build aracınca silinmiş olabilir; bu yüzden
 * dosya sisteminden değil git HEAD'inden okuyoruz. Yoksa null.
 */
function readDeployedVersion(distDir) {
  try {
    const content = sh.capture("git show HEAD:package.json", {
      cwd: distDir,
...
/**
 * Kaynak repo (deploy'un çalıştırıldığı klasör) package.json'unu dist
 * worktree'sine yazar:
 *  - dependencies / devDependencies / peerDependencies çıkarılır,
 *  - version, dist'te zaten yayınlanmış sürümün patch +1'i olur
 *    (ilk deploy'da kaynak version'dan başlar).
 * Kaynakta package.json yoksa hiçbir şey yapılmaz.
 */
function writeDistPackageJson(distDir, root, log, warn) {
  const srcPath = resolve(root, "package.json");
  if (!existsSync(srcPath)) {
    warn("Kaynakta package.json yok; dist package.json atlanıyor.");
    return;
  }

  let srcPkg;
  try {
    srcPkg = JSON.parse(readFileSync(srcPath, "utf8"));
  } catch (e) {
    warn(`Kaynak package.json okunamadı; atlanıyor: ${e.message}`);
    return;
  }

  const deployed = readDeployedVersion(distDir);
  const version = deployed ? bumpPatch(deployed) : srcPkg.version;

  // version anahtarını orijinal konumunda tutmak için spread + override.
  const { dependencies, devDependencies, peerDependencies, ...rest } = srcPkg;
  const distPkg = { ...rest, version };

  writeFileSync(resolve(distDir, "package.json"), `${JSON.stringify(distPkg, null, 2)}\n`);
  log(`package.json yazıldı (version: ${version}${deployed ? ` ← ${deployed}` : " ← kaynak"}).`);
}
...
export function deploy(userConfig) {
  ...
  // Kaynak package.json'u dist'e yaz (deps çıkarılmış, version patch +1).
  writeDistPackageJson(distDir, root, log, warn);
