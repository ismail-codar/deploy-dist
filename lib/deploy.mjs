...
function branchExistsLocal(branch, cwd) {
  try {
    sh.capture(`git show-ref --verify --quiet refs/heads/${branch}`, { cwd, stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
function ensureWorktree(distDir, cfg, root, log) {
  ...
  try {
    if (branchExistsLocal(cfg.branch, root) || branchExistsRemote(cfg.remote, cfg.branch, root)) {
      ...
      sh.run(`git worktree add "${cfg.dir}" ${cfg.branch}`, { cwd: root });
    } else {
      // Branch hiç yok: orphan branch + boş ilk commit.
      sh.run(`git worktree add --orphan -b ${cfg.branch} "${cfg.dir}"`, { cwd: root });
  ...
  // Yedekteki build dosyalarını worktree içine geri taşı.
  if (backup) {
    for (const entry of readdirSync(backup)) {
      renameSync(resolve(backup, entry), resolve(distDir, entry));
    }
...
export function deploy(userConfig) {
  ...
  const status = sh.capture("git status --porcelain", { cwd: distDir });
  if (!status) {
